...
will use binary-search tree properties from How to Specify
It!~\cite{Hughes2019HowTS}, which have been integrated into ETNA~\cite{ETNA} as case
studies. I will also use Rocq for the exposition because it is the language of our
implementation. Rocq hosts QuickChick~\cite{Pierce:SF4}, one of the most faithful ports
of QuickCheck, preserving much of QuickCheck's original machinery, types, and
algorithms.
...
other key. This property fails with a number of different bugs breaking
...
This is better, but ideally we would not generate the \code{k}, \code{k'}, and
\code{v} if the tree \code{t} does not pass the precondition \code{isBST}. The current
version still generates everything before starting to check. We could solve this by
turning to \code{forAllShrinkShowMaybe} (which is starting to look like
...
range of configurations for the user. All possible
...
introduce a variety of property runners from the literature that diverge from the
original property runner of QuickCheck illustrated in
Figure~\ref{fig:quickcheck-property-runner-intro}. These runners demonstrate the
motivation for a programmable PBT framework.
...
between different \emph{components}, such as
...
%
In the introduction, I showed the \code{Arbitrary} typeclass along with the
definition of \code{prop_sort_correct} using the \code{forAll} combinator QuickCheck
provides. In practice, QuickCheck is designed so users \emph{do not have to} write any
of those components by hand. This is possible via the
\code{Testable} typeclass. The top-level \code{quickCheck} function we saw is
defined as:
...
Automatic currying allows this typeclass instance to recursively handle functions
...
Conceptually, however, changing generators from \code{gen a :: Rng -> a} to
\code{gen a :: State -> Rng -> a} is enough to express a stateful property runner. In
Figure~\ref{fig:stateful-property-runner}, I present the stateful runner in the same
style as the previous runners in this section. The key
...
If we imagine shallowness and deepness as a spectrum, the opposite end from the
shallow embedding paradigm is a deep embedding in which terms of the host language are
...
staging. To my knowledge, there is only one tool
...
syntax that can include arbitrary Python expressions denoting a generative
...
\begin{enumerate}
  ...
  \item The first set of experiments demonstrates the expressiveness of DBAS, showing that it
        supports new property runners without requiring modifications to the
        library internals: we implemented all of the property runners presented in
        \S~\ref{sec:runners}. Here, I discuss four such runners and their implementations in
        detail: the standard QuickCheck-inspired property runner (\S~\ref{sec:loops:sgpr}), the
        mutation-based coverage-guided runner (\S~\ref{sec:loops:cgpr}), the mutation-based
...
can also be written directly by users, as shown in
...
interact with each other in order to show how users can implement new runners that fit
their needs.
...
Property-based testing has long relied on random generation from
...
Thus, we view the fuzzing property runner presented in Figure~\ref{fig:fuzzloop} as
...
and more complex properties than
simply ``the program does not
...
parallel testing runner. Parallelism is implemented through Racket futures. We use a lock-free
...
The configurability enabled by DBAS allows users to rely on a set of
community-accepted defaults chosen by framework developers, while also exploring
whether choices from the literature, or novel choices of their own, fit their testing
...
seed is reused. We conducted our experiments on the IFC workload in ETNA, using a
type-based generator alongside a type-based mutator. While this experiment reveals a
clear winner for this case study, the
...
other options fail to consistently solve a task across runs, indicating a
...
Test case reduction, counterexample minimization, or shrinking is a fundamental
aspect of random testing~\cite{zeller2002simplifying}; however, it receives much less
attention than random generation. A major split in PBT
...
that led to generation. This method, called integrated or internal shrinking, removes the
...
The central point of the study is that such splitting points should not be fixed at
the level of the library, but should instead be decisions made in
...
the authors provide an alternative, parallel property runner {\tt
    quickCheckPar}, in which multiple worker threads share a common variable that controls the
